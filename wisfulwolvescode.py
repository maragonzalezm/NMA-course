# -*- coding: utf-8 -*-
"""wisfulwolvesCode.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Bii_r4csB94Y9q9VKCH-ZBwUIzjr2uVk
"""
...
## Se usa para graficar una neurona en especifico y trial especifico, junto con el tiempo de stimulo, response time y feedback
#spikes=dat['spks']
...
## se crea un for para ejecutar los promedios de las actividades cerebrales x  region, en 3 diferentes tiempos x sesion
sessionmat=[]
for index in range(len(alldat)):
  dat=alldat[index]
  spikes=dat['spks']
  ## Se crea un vector con todos los valores del response como integers para su posterior utilizacion
  response=[]
  for index in range(len(dat['response_time'])):
    respns=np.round(dat['response_time'][index][0]*100).astype(int)
    response.append(respns)

  ##Se crea una lista donde se promedian los spikes para cada trial en los timebins de 50 ms antes de cada resopnse time, y se suman,
  ##dando como resultado un promedio de actividad cerebral por electrodo/region cerebral
  listaresp=[]
  ...
  ##Se crea una lista donde se promedian los spikes para cada trial en los timebins de 0 a 49 ms, y se suman,
  ##dando como resultado un promedio de actividad cerebral por electrodo/region cerebral (tomando como 0ms el inicio del trial y 50 ms la presentacion del estimulo)
  listastart=[]
  for i in range(len(spikes)):
    val2=np.sum(np.mean(spikes[i,:,:],axis=0)[0:49])
    listastart.append(val2)

  ##Se crea una lista donde se promedian los spikes para cada trial en los timebins de 50 a 100 ms, y se suman,
  ##dando como resultado un promedio de actividad cerebral por electrodo/region cerebral (tomando 50 ms como la presentacion del estimulo)
  listaafter=[]
  for i in range(len(spikes)):
...
#obtener la diferencia entre la act cerebral antes del movimiento y el baseline(antes del estimulo)
# 6, 12 y 39 tienen NaN
diff1=[]
diff2=[]
regions=[]
dictionary_final=[]
...
#crea un diccionario con los valores de diferencia en activacion cerebral para cada area, y posteriormente los ordena por area
dictionary_1=[]
for i in range(len(sessionmat)):
